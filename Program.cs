...
namespace hz
{
    ...
    public static class Program
    {
        ...
        private static readonly string DesktopPath = @"C:\Users\futile\Desktop\Lab08_Results";
        ...
        private static void SavePlot(string filePath, string title, string xLabel, string yLabel,
            double[] xValues, double[] theorValues, double[] expValues)
        {
            ...
            var scatterTheor = plt.Add.Scatter(xValues, theorValues);
            scatterTheor.Color = Colors.Blue;
            scatterTheor.Label = "Теоретическая";

            var scatterExp = plt.Add.Scatter(xValues, expValues);
            scatterExp.Color = Colors.Red;
            scatterExp.Label = "Экспериментальная";
        ...
        public static void Main()
        {
            ...
            Console.WriteLine($"\nРезультаты сохраняются в: {DesktopPath}");
            ...
            File.WriteAllLines(Path.Combine(DesktopPath, "results.txt"), reportLines);
            Console.WriteLine("\nОтчёт сохранён в: " + Path.Combine(DesktopPath, "results.txt"));

            double[] x = lambdaList.ToArray();
            SavePlot(Path.Combine(DesktopPath, "p-1.png"), "Вероятность простоя P0", "Интенсивность λ", "P0", x, theorP0.ToArray(), expP0.ToArray());
            SavePlot(Path.Combine(DesktopPath, "p-2.png"), "Вероятность отказа Pn", "Интенсивность λ", "Pn", x, theorPn.ToArray(), expPn.ToArray());
            SavePlot(Path.Combine(DesktopPath, "p-3.png"), "Относительная пропускная способность Q", "Интенсивность λ", "Q", x, theorQ.ToArray(), expQ.ToArray());
            SavePlot(Path.Combine(DesktopPath, "p-4.png"), "Абсолютная пропускная способность A", "Интенсивность λ", "A (заявок/сек)", x, theorA.ToArray(), expA.ToArray());
            SavePlot(Path.Combine(DesktopPath, "p-5.png"), "Среднее число занятых каналов k", "Интенсивность λ", "k", x, theorK.ToArray(), expK.ToArray());

            Console.WriteLine("\nГрафики сохранены:");
            Console.WriteLine($"  {Path.Combine(DesktopPath, "p-1.png")}");
            Console.WriteLine($"  {Path.Combine(DesktopPath, "p-2.png")}");
            Console.WriteLine($"  {Path.Combine(DesktopPath, "p-3.png")}");
            Console.WriteLine($"  {Path.Combine(DesktopPath, "p-4.png")}");
            Console.WriteLine($"  {Path.Combine(DesktopPath, "p-5.png")}");
